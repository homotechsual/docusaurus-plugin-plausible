...
* Changed plugin return type from `Plugin<void>` to `Plugin<unknown>` to satisfy Docusaurus's `PluginModule<unknown>` contract and resolve a TypeScript assignment error in consumer `docusaurus.config.ts` files
